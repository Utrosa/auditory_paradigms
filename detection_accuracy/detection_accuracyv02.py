...
def create_and_preload_tones(trials, tone_duration, tone_frequency, tone_samplerate, tone_bitdepth):
	"""
	Trials is a random sample of valid parameter combinations (len(NO_TRIALS)).
	"""
...
for run_combo in RUN_COMBOS:
	# For a block of trials, create and preload tones
	...
	tones_all.append(tones_by_trial)
...
for TRIAL_PARAMS in RUN_COMBOS:
	...
	for rep, trial in enumerate(TRIAL_PARAMS):
		...
		for sequence in trial_tones:
			exp.clock.wait(params["ITI"])

			# Play sequence
			for count, t in enumerate(sequence):
				t.play()
