...
# Script for Expyriment version 1.0.0
...
params = {
	...
	"LOC_REP"     	   		: 4,  # the number of functional MRI sequences
	...
	"INTRO_TEXT_LOC" 	  : f"You will hear sequences of different sounds, separated by longer periods of silence."
...
instructions_loc  = stimuli.TextScreen(params["INTRO_HEADING"], params["INTRO_TEXT_LOC"], heading_size = params["HEADING_SIZE"], text_size = params["TEXT_SIZE"])
...
canvas.preload(); instructions_loc.preload(), scanner_text.preload()
...
soundtrack = create_soundtrack(sound_strata = list(sound_strata.values()), sequence_len = params["SOUNDS_PER_SEQUENCE"],
								rep_prob = params["SOUND_REP_PROB"], sequence_no = params["LOC_TRIALS"] * params["LOC_REP"])
...
# Wait for the participant to read the instructions_loc.
...
for run in range(params["LOC_REP"]): # Runs == Blocks == Functional Acquisitions
