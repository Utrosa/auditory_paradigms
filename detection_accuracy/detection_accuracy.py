...
def create_and_preload_tones(trials, tone_duration, tone_frequency, tone_samplerate, tone_bitdepth):
    ...
    for trial in trials:
        ...
        tones_by_trial.append(trial_tones)
...
params = {
	...
	"NO_TRIALS" : 2,         # A trial is one tone sequence
...
DEV      = random.sample(range(10, 50), 3) # TODO: adapt these test deviations
DEV.insert(0, 0) 
...
ISI      = random.sample(range(300, 600), 3)
...
# If we would want to select trials to ensure more or less the same duration of blocks.
# paired_trials = list(zip(VALID_COMBOS, COMBO_DURATIONS))
# random.shuffle(paired_trials)

# TRIAL_PARAMS: a sequence of tones, repecting the parameters given in the tuple of VALID_COMBOS.
TRIAL_PARAMS = random.sample(VALID_COMBOS, params["NO_TRIALS"])
...
cross.preload()
tones_by_trial = create_and_preload_tones(
	TRIAL_PARAMS,
