...
def create_experimental_sessions(params, sesID, save_csv=False, MAX_BLOCK_DURATION_MIN=15):
	...
	# Verify the distribution trials.
	...
	if save_csv:
		...
		print(f"\nSaved {filename} to {out_path}.")	# 01. GENERATE INDEPENDENT VARIABLES: Timing deviation size and location ----------------------
	# Create negative values of tone's timing deviation. If applicable, includes a "negative" zero.
	DEV_pos = np.array(params["DEVS"])
	DEV_neg = -DEV_pos
...
if __name__ == "__main__":
	...
	for session in range(4):
		with open(f"trials/exp_parameter_combo_ses-{session:003d}.txt", "w") as f:
			sys.stdout = f
