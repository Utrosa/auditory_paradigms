...
def play_sounds(soundtrack, duration):
	...
	for sound_ID in soundtrack:
		...
		if sound_ID in sounds_trial and key == '1': # HIT
			...
			exp.clock.wait(params["WAIT_TIME"])
			perfo_trial["M"] += 1
		elif sound_ID not in sounds_trial and key == '4': # CORRECT REJECTION
			correct.present()
			exp.clock.wait(params["WAIT_TIME"])
			perfo_trial["CR"] += 1
...
params = {
	...
	"SOUNDS_PER_TRIAL"   : 10,
	"WAIT_TIME"          : 500,   # milliseconds (the duration of the pause between sound and question)
	...
	"SOUND_REP_PROB"   : .1,
	"SILENCE_DURATION" : 30000 # milliseconds
}
